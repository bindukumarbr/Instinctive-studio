...
export default function FilterPanel({ facets, categoryAttributeSchema, currentCategorySlug }: FilterPanelProps) {
    ...
    const [activeFilters, setActiveFilters] = useState<Record<string, string | string[]>>(getInitialActiveFilters);
    ...
    const handleFilterChange = (filterName: string, value: string, type: 'checkbox' | 'radio' | 'text') => {
    ...
    useEffect(() => {
        ...
        const filtersHaveChanged = JSON.stringify(activeFilters) !== JSON.stringify(currentUrlFilters);
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            <button
                ...
                `}
                disabled={Object.keys(activeFilters).length === 0 && !isCategorySelected}
            >
                Clear All Filters
            </button>
            ...
            {!isCategorySelected ? (
                ...
                <div>
                    ...
                    {Object.keys(facets).length > 0 && (
                        <div className="mb-6">
                            <h3 className="text-lg font-semibold mb-2 text-gray-700">General Filters</h3>
                            {Object.entries(facets).map(([facetName, values]) => (
                                <div key={facetName} className="mb-4">
                                    <h4 className="text-md font-medium capitalize mb-2">{facetName.replace(/_/g, ' ')}</h4>
                                    {values.map((facetItem, index) => (
                                        <div key={facetItem._id || index} className="flex items-center mb-1">
                                            <input
                                                type="checkbox"
                                                ...
                                                onChange={() => handleFilterChange(facetName, facetItem._id, 'checkbox')}
                                                className="mr-2 h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                                            />
                                            <label htmlFor={`facet-<span class="math-inline">\{facetName\}\-</span>{facetItem._id}`} className="text-gray-700 cursor-pointer">
                    ...
                    {currentCategorySchema && currentCategorySchema.attributes.length > 0 && (
                        <div className="mb-6">
                            <h3 className="text-lg font-semibold mb-2 text-gray-700">Category Attributes ({currentCategorySchema.categoryName})</h3>
                            {currentCategorySchema.attributes.map(attr => (
                                <div key={attr.key} className="mb-4">
                                    <h4 className="text-md font-medium capitalize mb-2">{attr.name}</h4>
                                    {attr.type === 'enum' && attr.options && attr.options.length > 0 ? (
                                        <div>
                                            {attr.options.map((option, index) => (
                                                <div key={index} className="flex items-center mb-1">
                                                    <input
                                                        ...
                                                        onChange={() => handleFilterChange(attr.key, option, 'checkbox')}
                                                        className="mr-2 h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                                                    />
                                                    <label htmlFor={`attr-<span class="math-inline">\{attr\.key\}\-</span>{option}`} className="text-gray-700 cursor-pointer">
                                                        {option}
                                                    </label>
                                                </div>
                                            ))}
                                        ...
                                        <p className="text-sm text-gray-500 italic">No filter options available for this attribute.</p>
                                    )}
                                </div>
                            ))}
                        </div>
                    ...
                    {Object.keys(facets).length === 0 && (!currentCategorySchema || currentCategorySchema.attributes.length === 0) && (
                        <div className="p-4 bg-yellow-50 border border-yellow-200 text-yellow-800 rounded-md text-sm text-center animate-fadeIn">
                            ...
                            <p>There are no specific filters for the current selection or category.</p>
                        </div>
                    )}
                </div>
            )}
